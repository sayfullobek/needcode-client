...
import { LESSON_BREADCRUMP } from '../../../utils/BreadcrumpUtils'
import { LESSON_HEAD } from '../../../utils/TableHeadUtils'
...
export const GoModule = () => {
	...
	const getAll = async (page = 1, limit = 10) => {
		try {
			const res = await AutoGet(
				`${APP_API.lesson}?page=${page}&limit=${limit}&moduleId=${moduleId}`
			)
			setData(res.data)
			setTotalItems(res.total)
			const searchs = await AutoGet(
				`${APP_API.lesson}?page=1&limit=${res.totalItems}&moduleId=${moduleId}`
			)
	...
	const exportToExcel = () => {
		...
		XLSX.utils.book_append_sheet(workbook, worksheet, 'LessonData')
	...
	return loading ? (
		<Box sx={{ padding: '3rem' }}>
			<Breadcrump
				status={'addAndBack'}
				url={`/${DASHBOARD_URL.lessonAdd}/${courseId}/${moduleId}`}
				backUrl={`/${DASHBOARD_URL.goCourse}/${courseId}`}
				arr={LESSON_BREADCRUMP(courseId, 'Kurs', 'Modul')}
			/>
			<Box
				sx={{
					display: 'flex',
			...
			<Box sx={{ width: '100%', display: 'flex' }}>
				<AutoTable
					...
					fields={LESSON_HEAD}
					url={DASHBOARD_URL.lessonUpdate}
					deleteUrl={APP_API.lesson}
					getAll={getAll}
					pathName={pathName}
				/>
			</Box>
