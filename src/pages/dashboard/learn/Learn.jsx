...
import { LEARN_BREADCRUMP } from '../../../utils/BreadcrumpUtils'
import { LEARN_HEAD } from '../../../utils/TableHeadUtils'
...
export const Learn = () => {
	...
	const getAll = async (page = 1, limit = 10) => {
		try {
			const res = await AutoGet(`${APP_API.learn}?page=${page}&limit=${limit}`)
			setData(res.data)
			setTotalItems(res.totalItems)
			const searchs = await AutoGet(
				`${APP_API.learn}?page=1&limit=${res.totalItems}`
			)
	...
	const exportToExcel = () => {
		...
		XLSX.utils.book_append_sheet(workbook, worksheet, 'LearnData')
		...
		saveAs(dataBlob, 'learn-data.xlsx')
	}
	...
	return loading ? (
		<Box sx={{ padding: '3rem' }}>
			<Breadcrump
				status={'add'}
				url={`/${DASHBOARD_URL.learnAdd}`}
				arr={LEARN_BREADCRUMP}
			/>
			<Box
				sx={{
					display: 'flex',
			...
			<AutoTable
				...
				fields={LEARN_HEAD}
				url={DASHBOARD_URL.learnUpdate}
				deleteUrl={APP_API.learn}
				getAll={getAll}
				pathName={pathName}
			/>
