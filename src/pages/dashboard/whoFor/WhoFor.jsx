...
import { WHO_FOR_BREADCRUMP } from '../../../utils/BreadcrumpUtils'
import { WHO_FOR_HEAD } from '../../../utils/TableHeadUtils'
...
export const WhoFor = () => {
	...
	const getAll = async (page = 1, limit = 10) => {
		try {
			const res = await AutoGet(`${APP_API.whoFor}?page=${page}&limit=${limit}`)
			setData(res.data)
			setTotalItems(res.totalItems)
			const searchs = await AutoGet(
				`${APP_API.whoFor}?page=1&limit=${res.totalItems}`
			)
	...
	const exportToExcel = () => {
		...
		XLSX.utils.book_append_sheet(workbook, worksheet, 'whoForData')
		...
		saveAs(dataBlob, 'who-for-data.xlsx')
	}
	...
	return loading ? (
		<Box sx={{ padding: '3rem' }}>
			<Breadcrump
				status={'add'}
				url={`/${DASHBOARD_URL.whoForAdd}`}
				arr={WHO_FOR_BREADCRUMP}
			/>
			<Box
				sx={{
					display: 'flex',
			...
			<AutoTable
				...
				fields={WHO_FOR_HEAD}
				url={DASHBOARD_URL.whoForUpdate}
				deleteUrl={APP_API.whoFor}
				getAll={getAll}
				pathName={pathName}
			/>
